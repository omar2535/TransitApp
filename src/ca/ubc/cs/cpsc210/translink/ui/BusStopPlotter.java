...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        while(iterator.hasNext()) {
            ...
            LatLon point = s.getLocn();


             if (Geometry.rectangleContainsPoint(northWest, southEast, point) && !stopMarkerMap.containsKey(s)) {
                 ...
                 String allRoutes = s.getName() + "\n";
                 for(Route r: routes){
                     allRoutes += "route " + r.getNumber() + "\n";
                 }
                 marker.setTitle(allRoutes);
                setMarker(s, marker);
                stopClusterer.add(getMarker(s));
            }else if(stopMarkerMap.containsKey(s) && stopMarkerMap.get(s) == nearestStnMarker){
                 String allRoutes = s.getName() + "\n";
                 for(Route r: s.getRoutes()){
                     allRoutes += "route" + r.getNumber() + "\n";
                 }
                 nearestStnMarker.setTitle(allRoutes);
             }
        }
    }
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        if(nearest == null) {
            ...
            if (stopMarkerMap.containsKey(nearest)) {
                ...
                setMarker(nearest, nearestStnMarker);
                stopClusterer.add(getMarker(nearest));
