...
public class BusLocationPlotter extends MapViewOverlay {
    ...
    public void plotBuses() {
        this.busLocationsOverlay.removeAllItems();
        Stop selectedStop = StopManager.getInstance().getSelected();
        if(selectedStop != null) {
            List<Bus> listofBusses = selectedStop.getBuses();
            for (Bus bus : listofBusses) {
                OverlayItem busitem = new OverlayItem(bus.getRoute().getName(), bus.getRoute().getNumber(), Geometry.gpFromLL(bus.getLatLon()));

                this.busLocationsOverlay.addItem(busitem);
            }
        }
    }
