...
public class ArrivalsListFragment extends ListFragment {
    ...
    private ArrayList<Arrival> getArrivalsForSelectedStop() {
        ...
        Iterator arrivalsIterator = selectedStop.iterator();

        while(arrivalsIterator.hasNext()){
            Arrival arrival = (Arrival)arrivalsIterator.next();
            if(arrival.getTimeToStopInMins() <=0){
